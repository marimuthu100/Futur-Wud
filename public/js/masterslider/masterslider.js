...
(function ($) {
    ...
    switch (this.swipeType) {
      ...
      case "all":
        if (Math.abs(new_x - this.start_x) > Math.abs(new_y - this.start_y))
          return new_x <= this.start_x ? "left" : "right";
        else return new_y <= this.start_y ? "up" : "down";
        break;
    }
  };
  ...
  p.priventDefultEvent = function (new_x, new_y) {
    ...
    return (
      (this.swipeType === "horizontal" && horiz) ||
  ...
  p.__reset = function (event, jqevt) {
    ...
    this.start_x = isTouch
      ? event.touches[0].pageX
      : usePointer
      ? event.pageX
      : jqevt.pageX;
    this.start_y = isTouch
  ...
  p.__touchStart = function (event) {
    ...
    if (
      $(event.target).closest(swipe.noSwipeSelector, swipe.$element).length > 0
    ) {
      return;
    }
    ...
    if (usePointer) {
      $(this).css(
        "-ms-touch-action",
        swipe.swipeType === "horizontal" ? "pan-y" : "pan-x"
      );
    }
    ...
    if (properties.x !== undefined || properties.y !== undefined) {
      if (_cssanim) {
        ...
        if (properties.x !== undefined) {
          properties[trans] =
            (properties[trans] || "") + " translateX(" + properties.x + "px)";
          delete properties.x;
        }

        if (properties.y !== undefined) {
          properties[trans] =
            (properties[trans] || "") + " translateY(" + properties.y + "px)";
          delete properties.y;
        }
      } else {
        if (properties.x !== undefined) {
  ...
  p.changeTo = function (value, animate, speed, snap_num, dispatch) {
    ...
    if (animate) {
      ...
      var self = this,
        ...
        timeconst =
          animFrict + ((speed - 20) * animFrict * 1.3) / self.options.maxSpeed;
      ...
      var tick = function () {
        ...
        self.value =
          targetPosition - amplitude * Math.exp(-++timeStep * timeconst);
...
(function ($) {
  ...
  p.create = function () {
    ...
    if (this.parallax != null) {
      ...
      this.alignedToBot =
        this.layerOrigin && this.layerOrigin.indexOf("b") !== -1;
  ...
  p.locate = function () {
    ...
    for (var key in this.baseStyle) {
      isPosition =
        key === "top" || key === "left" || key === "bottom" || key === "right";
      ...
      if (key === "top" && this.middleAlign) {
        ...
        this.$element[0].style.left =
          this.baseStyle["left"] * factor + (width - this.baseWidth) / 2 + "px";
      } else {
    ...
    switch (this.align) {
      case "top":
        ...
        if (this.base_l < 0) {
          ...
          this.tt_arrow.css(
            "margin-left",
            -arrow_w / 2 + this.pos_x - this.tt.outerWidth(false) / 2 + "px"
          );
        }
  ...
  p.setBGVideo = function ($video) {
    ...
    if (this.bgvideo_fillmode !== "none") {
      ...
      this.bgvideo.addEventListener("loadedmetadata", function () {
        ...
        that.vinit = true;
        that.video_aspect =
          that.bgVideoAligner.baseHeight / that.bgVideoAligner.baseWidth;
...
(function ($) {
  ...
  p.onTimer = function (time) {
    ...
    if (
      this.so.hideLayers &&
      !this.hideCalled &&
      this.view.currentSlide.delay * 1000 - this._timer.getTime() <= 300
    ) {
...
(function ($) {
  ...
  p.__setupSlides = function () {
    ...
    this.$element.children(".ms-slide").each(function (index) {
      ...
      new_slide.delay =
        $slide_ele.data("delay") !== undefined ? $slide_ele.data("delay") : 3;
      new_slide.fillMode =
        $slide_ele.data("fill-mode") !== undefined
  ...
  p.__createSlideLayers = function (slide, layers) {
    ...
    layers.each(function (index, domEle) {
      ...
      if (
        domEle.nodeName === "A" &&
        $layer_element.find(">img").data("type") === "image"
      ) {
        $parent_ele = $(this);
  ...
  p.setup = function (target, options) {
    ...
    if ($.browser.msie) {
      this.$element
        .addClass("ms-ie")
        .addClass(
          "ms-ie" + $.browser.version.slice(0, $.browser.version.indexOf("."))
        );
    } else if ($.browser.webkit) {
  ...
  $.fn[pluginName] = function (options) {
    ...
    if (options === undefined || typeof options === "object") {
      ...
      this.each(function () {
        ...
        if (
          instance instanceof MasterSliderPlugin &&
          typeof instance._slider.api[options] === "function"
        ) {
...
(function ($) {
  ...
  p.__getSteps = function (index, target) {
    var right =
      target < index ? this.slidesCount - index + target : target - index;
...
(function ($) {
  ...
  p.__updateSlidesHoriz = function (slide, distance) {
    ...
    slide.$element.css(
      ...
      "translateZ(" +
        -value * 3 +
        "px) rotateY(0.01deg)" /* translateX('+(distance < 0 ? 1 : -1) * -value * 5+'px)'*/
    );
  };
...
(function ($) {
  ...
  p.__updateSlidesHoriz = function (slide, distance) {
    ...
    slide.$element[0].style[window._jcsspfx + "Transform"] =
      "translateZ(" + -zvalue * 5 + "px) rotateY(" + rvalue + "deg) ";
  };
  ...
  p.__updateSlidesVertic = function (slide, distance) {
    ...
    var zvalue = value * 1.2;
    slide.$element[0].style[window._jcsspfx + "Transform"] =
      "translateZ(" + -zvalue * 5 + "px) rotateX(" + -rvalue + "deg) ";
  };
...
(function ($) {
  ...
  p.__updateSlides = function (slide, distance) {
    ...
    if (1 - value <= 0) {
      ...
      if (distance < 0) {
        element.style[window._jcsspfx + "Transform"] =
          "perspective(2000px) translateZ(" + value * -300 + "px)";
      } else {
        element.style[window._jcsspfx + "Transform"] =
  ...
  p.__updateSlidesHoriz = function (slide, distance) {
    ...
    slide.$element.css(
      ...
      "translateZ(" +
        ...
        (distance < 0 ? 1 : -1) *
          (-this.__calcview(value, this.__width) * this.options.centerSpace) +
        "px)"
    );
  };
...
(function () {
  ...
  p.__updateSlidesHoriz = function (slide, distance) {
    ...
    if (slide.hasBG) {
      slide.$bg_img.css(
        "opacity",
        (100 - Math.abs((distance * 120) / this.__width / 3)) / 100
      );
    }
    slide.$element.css(
      window._csspfx + "transform",
  ...
  p.checkHideUnder = function () {
    if (this.options.hideUnder) {
      ...
      this.needsRealign =
        ...
        (this.options.align === "left" || this.options.align === "right") &&
...
(function ($) {
  ...
  p.align = function (event) {
    ...
    var align = this.options.align;
    var pos = this.slider.reserveSpace(
      align,
  ...
  p.slideAction = function (slide) {
    ...
    if (this.options.align) {
      thumb_frame
        .width(
          this.options.width -
            (this.options.dir === "v" && this.options.type === "tabs" ? 12 : 0)
        ) // less arrow size 12px
  ...
  p.vertSwipeMove = function (status) {
    ...
    var phase = status.phase;
    if (phase === "start") this.controller.stop();
    else if (phase === "move") this.controller.drag(status.moveY);
    else if (phase === "end" || phase === "cancel") {
  ...
  p.horizSwipeMove = function (status) {
    ...
    } else if (phase === "move") this.controller.drag(status.moveX);
    else if (phase === "end" || phase === "cancel") {
  ...
  p.__resize = function () {
    ...
    if (len <= size) {
      this.dTouch = true;
      this.controller.stop();
      this.$thumbscont[0].style[this.__pos] = (size - len) * 0.5 + "px";
      this.$thumbscont[0].style[window._jcsspfx + "Transform"] = "";
    } else {
      this.dTouch = false;
      this.click_enable = true;
...
(function ($) {
  ...
  p.create = function () {
    ...
    this.$element.click(function () {
      if (that.slider.api.paused) that.slider.api.resume();
      else that.slider.api.pause();
    });
...
(function ($) {
  ...
  window.MSFlickrV2 = function (slider, options) {
    ...
    if (this.options.type === "photoset") {
      ...
      $.getJSON(
        getUserPublicURL(this.options.key, this.options.id, this.options.count),
        function (data) {
  ...
  p.moveParallax = function (e) {
    ...
    if (out <= 0) {
      if (layers) {
        if (window._css3d) {
          layers[0].style[window._jcsspfx + "Transform"] =
            "translateY(" + -out * that.parallax + "px) translateZ(0.4px)";
        } else if (window._css2d) {
          layers[0].style[window._jcsspfx + "Transform"] =
...
(function (document, window, jQuery) {
  ...
  p.init = function () {
    ...
    this.target =
      this.slider.options.filterTarget === "slide" ? "$element" : "$bg_img";
